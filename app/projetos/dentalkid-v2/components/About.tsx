...
export default function About() {
  ...
  return (
    <section className="py-24 bg-white relative overflow-hidden" id="sobre">
      {/* decorative blobs */}
      <div className="absolute top-0 right-0 w-64 h-64 bg-sky-50 rounded-full -translate-y-1/2 translate-x-1/2 opacity-60" />
      <div className="absolute bottom-0 left-0 w-48 h-48 bg-green-50 rounded-full translate-y-1/2 -translate-x-1/2 opacity-60" />
        ...
        >
          {stats.map((stat) => (
            <motion.div
              ...
              className="text-center bg-gradient-to-br from-sky-50 to-green-50 rounded-3xl p-6 border border-sky-100 hover:shadow-lg transition-shadow duration-300"
            >
              <div className="text-4xl mb-2">{stat.emoji}</div>
              <div
                className="text-3xl font-black text-sky-700 mb-1"
                style={{ fontFamily: "'Nunito', sans-serif" }}
              >
                {stat.value}
              </div>
